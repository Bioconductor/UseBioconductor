<!--
%\VignetteIndexEntry{01.5 Annotating Genes, Genomes,and Variants}
...
- Coordinate mappping using chain files and 
  [`liftOver()`](http://bioconductor.org/help/newsletters/2015_January/#coordinate-mapping)

Example 2:

- `grasp2db` package (under development)

```{r grasp2db, eval=FALSE}
...
- _Bioconductor_ annotation packages are based on _sqlite_ databases.
- `r CRANpkg("dplyr")` provides convenient access via `src_sqlite`,
  `tbl`, `filter`, `arrange`, `summarize`, etc.

Exercise: use `AnnotationDbi::dbfile()` on a TxDb object `gene` table
...
'Easy' to write web-based services, e.g., to query ENSEMBL
[REST](http://rest.ensembl.org/) API
...
```{r ensembl-ping, eval=FALSE}
.get('/info/ping')$ping
```

- Symbol -> ENSG identifier mapping

```{r ensembl-mapping, eval=FALSE}
...
[Homo.sapiens]:
http://bioconductor.org/packages/release/data/annotation/html/Homo.sapiens.html
[Rsamtools]:
...
[org.Sc.sgd.db]:
http://bioconductor.org/packages/release/data/annotation/html/org.Sc.sgd.db.html
[rtracklayer]:
